    ...
    def _on_receive(self, _event: Event or None) -> None:
        ...
        if msg:
            ...
            if self.socket is None:
                ...
                origmsg = msg
                # translate message
                msgTranslated = translator.translate(msg, dest="en")
                lang = "en"
                ...
                if category == "question":
                    subject: str = ' '.join([word for word in origmsg.split() if word.lower() not in removableWords])
                    if len(subject.strip()) == 0:
                        response: str = "Can you be more specific?"
                    else:
                        try:
                            response: str = wiki.summary(subject, sentences=1)
                        except:
                            response: str = "Can you be more specific? " + subject.title() + " can mean many things."
                else:
                    subject: str = fs(msg)
                    response: str = gs(category, subject)
